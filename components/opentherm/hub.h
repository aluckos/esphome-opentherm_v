...
class OpenthermHub : public Component {
 ...
 public:
  ...
  void send_raw_message(OpenthermData &data) {
    if (this->opentherm_ != nullptr) {
      this->opentherm_->send(data);
      this->last_conversation_start_ = millis();
      this->last_mode_ = WRITE;
  ...
  bool ch_enable = true, dhw_enable = true, cooling_enable = false, otc_active = false, ch2_active = false, summer_mode_active = false, dhw_block = false;
  ...
  void add_on_before_send_callback(std::function<void(OpenthermData &)> &&callback) { this->before_send_callback_.add(std::move(callback)); }
  void add_on_before_process_response_callback(std::function<void(OpenthermData &)> &&callback) { this->before_process_response_callback_.add(std::move(callback)); }
